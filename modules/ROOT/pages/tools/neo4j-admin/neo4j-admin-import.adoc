...
----
neo4j-admin database import full <database> [-h] [--expand-commands] [--verbose] [--auto-skip-subsequent-headers[=true|false]]
...
.`neo4j-admin database import full <database>` parameters
...
Valid formats are `standard`, `aligned`, `high_limit`, `block`.
...
[source, shell]
--
bin/neo4j-admin database import full --format=high_limit
--
...
[source, shell]
----
bin/neo4j-admin database import full mydb @/path/to/your/<args-filename> 
----
...
For more information, see <<import-files-from-aws-s3-uris>>.
====
...
[source, shell, role=noplay]
----
bin/neo4j-admin database import full databasename --nodes import/node_header.csv,'import/node_data_\d{1,5}.csv'
----

[[import-files-from-aws-s3-uris]]
==== Importing files from an AWS S3 storage

_This feature is available from Neo4j 5.19._

To import data stored in an AWS S3 storage, you can use the `--nodes` and `--relationships` options with S3 URIs.
...
. Run the `neo4j-admin database import` command to import your database from the AWS S3 storage:
+
[source, shell, role=noplay]
----
bin/neo4j-admin database import full newdb --nodes s3://myBucket/data/nodes.csv --relationships s3://myBucket/data/relationships.csv
----
